...
import { createJobsForSavedDomains, createJobsForSearch } from './job-producer.js';
  ...
  try {
    ...
    if (webhook) {
      if (webhook.includes('adv')) {
        ...
        await sendWebhook(webhook, {
          domain: term, // This is different from above
          format,
          stats,
        }, webhookMethod);
      }
...
server.post('/create-crawl-jobs/domain', async (req, res) => {
  ...
  try {
    const detail = await createJobsForSearch(domain, webhook, webhookMethod);
